// /api/order  -- 버셀 서버리스 함수 (주문 생성 프록시, SHL 버디케인과 동일 방식)
// edition.rudix.biz 랜딩에서 same-origin으로 호출 -> 여기서 아임웹(rudix.biz)에 서버-투-서버로 요청
...
export default async function handler(req, res) {
  ...
  try {
    // 아임웹 OMS_add_order 에 보낼 body (실제 구매하기 요청과 동일한 파라미터)
    const params = new URLSearchParams();
    params.append('backurl', 'https://rudix.biz/shop_view/?idx=14');
    params.append('prodIdx', '14');
    ...
    params.append('shipping_template_code', 'T202509176366fdaacabfb');
    params.append('infoUrl', 'https://rudix.biz');

    const imwebRes = await fetch('https://rudix.biz/shop/oms/OMS_add_order.cm', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
        'x-requested-with': 'XMLHttpRequest',
        'imweb-landing-url': 'https://rudix.biz/',
        'referer': 'https://rudix.biz/shop_view/?idx=14'
      },
    ...
    if (data && data.back_url_base64) {
      ...
      const paymentUrl = 'https://rudix.biz' + path;
      res.status(200).json({
        ok: true,
